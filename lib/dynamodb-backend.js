...
DynamoDBBackend.prototype = {
    ...
    */
    get: function(bucket, key, cb) {
        ...
        key = encodeText(key);
        var tableName = this.prefix + (this.useSingle ? aclTableName : getTableName(bucket));
        var params = {
            TableName: tableName,
            Key: {
                key: key
    ...
    */
    union: function(bucket, keys, cb) {
        ...
        var tableName = self.prefix + (self.useSingle ? aclTableName : getTableName(bucket));
...
function encodeText(text) {
    if (typeof text === 'string' || text instanceof String) {
        // TODO: decide if this encode is actually needed, and if so resolve tests 
...
function getTableName(bucket) {
    // TODO: handle names longer than 255 chars... maybe use a hashcode in those cases instead?
